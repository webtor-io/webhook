...
// The catalog contract web-ui relies on: both lists are arrays even when the
// tables are empty (a missing "tiers" key means an older webhook), and an
// unlimited tier carries null — never 0, which would read as "no Vault".
func TestListPricesResponseJSON(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			resp: newListPricesResponse(nil, nil),
			want: `{"prices":[],"tiers":[]}`,
		},
		{
